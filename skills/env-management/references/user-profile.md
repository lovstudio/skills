...
For `lov-env-management`, safe durable values include the default backend name,
expiry-warning days, zsh rc path, and whether to open the local Dashboard.
API Keys, secret references, fingerprints, probe headers, and generated exports
must remain in the dedicated credential store and never enter this Profile.
