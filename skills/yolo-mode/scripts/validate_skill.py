...
def validate_runtime_manifest(
    ...
    if not isinstance(interaction, dict):
        ...
        valid_counts = range(1, 4) if interaction.get("ask_missing") else (0,)
        if not isinstance(max_questions, int) or isinstance(max_questions, bool) or max_questions not in valid_counts:
            errors.append(f"{manifest_path}: interaction.max_questions must be 1-3 when ask_missing is true, otherwise 0")
