#!/usr/bin/env python3
"""Read-only ffprobe preflight for videos uploaded to WeChat Channels."""
...
def validate_probe(
    ...
    *,
    ...
    if size_bytes <= 0:
        ...
        errors.append(
            _issue("file_too_large", f"文件超过 {max_gb:g} GiB 硬限制")
        )
    ...
    if duration is None:
        ...
        errors.append(_issue("duration_too_short", "视频时长必须至少为 3 秒"))
    elif duration > max_duration:
        errors.append(
            _issue("duration_too_long", f"视频时长超过 {max_hours:g} 小时硬限制")
        )
...
def _failure_result(
    path: Path, message: str, *, max_gb: float, max_hours: float
) -> Dict[str, Any]:
    return {
        "valid": False,
        "status": "fail",
...
def check_video(
    ...
    *,
    ...
    if not path.exists():
        return _failure_result(path, "视频文件不存在", max_gb=max_gb, max_hours=max_hours)
    if not path.is_file():
        return _failure_result(path, "输入路径不是文件", max_gb=max_gb, max_hours=max_hours)
    try:
        size_bytes = path.stat().st_size
        probe = run_ffprobe(path, ffprobe=ffprobe)
    except (OSError, ProbeError) as exc:
        return _failure_result(path, str(exc), max_gb=max_gb, max_hours=max_hours)
...
def format_human(result: Dict[str, Any]) -> str:
    ...
    lines = [
        f"视频发布预检：{'通过' if result['valid'] else '未通过'}",
        f"文件：{result['path']}",
        (
            "检测："
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="只读检查视频号上传视频")
