...
截至 2026 年 7 月 13 日，最直接的官方依据是 [Anthropic《Skills for enterprise》](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/enterprise)。它明确讨论了大规模 Skills 的召回限制、共存评测、内部 Registry、角色 Bundle、版本控制与退役。OpenAI 和 Agent Skills 开放标准则补充了上下文预算、作用域、渐进披露和用户侧安全边界。
...
一个可持续的 Skill 系统至少需要四层：

```text
Registry：保存全部 Skill、版本、关系和评测
  ↓
Scope / Bundle：按用户、项目、角色、权限和风险过滤
  ↓
Router：召回少量候选、重排、判断是否需要澄清
  ↓
Runtime：只加载命中的 SKILL.md、脚本和参考资料
```
...
管理 100+ Agent Skills，重点已经不再是继续写更多 `SKILL.md`，而是建立一个小型操作系统：Registry 负责资产，Scope 与 Bundle 负责边界，Router 负责选择，Runtime 负责按需加载，Eval 与 Lifecycle 负责让系统可以持续演进。
