...
兼容旧命令的中文表层模式扫描器。它只定位套话、过渡词和句段均匀等可观察特征，
不判断作者身份，不预测平台审核，也不提供规避 AI 标识或检测的能力。

> Deprecated：新的“去 AI 味”、作者性审计、篇章改写和复测任务请使用
> `lov-human-writing`。本仓库只保留 `scripts/analyze.py` 的旧命令兼容。
...
| 50-100 | HIGH | 旧版表层规则命中较多，不代表 AI 概率或平台风险 |
