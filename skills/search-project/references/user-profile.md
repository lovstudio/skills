...
## lov-search-project 特有字段

本 Skill 依赖 Profile 中的 `workspace.projects`（字符串数组）作为额外搜索根。
用户说出「项目都在 /xxx」时持久化方式：

```bash
python3 scripts/profile_store.py record \
  --skill-id lov-search-project \
  --path workspace.projects \
  --value '["$HOME/projects","$HOME/Documents"]' \
  --confirm
```

`scripts/find_project.py` 每次运行读取 `SKILL_PROFILE_PATH` 指向的 Profile，
将 `workspace.projects` 追加到默认搜索根。未配置时使用内置默认根
（~/projects、~/Documents、~/lovstudio、~/yoda、~/Desktop）。

**注意**：`find_project.py` 的默认根与 profile 读取逻辑位于同一文件内，保持
可移植——默认根是通用推断值，个人路径一律从 Profile 或运行时环境注入。
